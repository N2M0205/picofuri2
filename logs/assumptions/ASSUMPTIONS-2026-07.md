...
### [2026-07-07 判明] Yahoo IP ペナルティ の解除時間 (実測サンプル1)

- **種別**: 🔎知見の記録
- **背景**: 2026-07-06 20:30 の Stage 3 (20kw) で 429 発生、breaker 発動。
  即時 (4分後) の Stage 2 (8kw) 再開試行でも restart 直後 14秒で 429 再発。
  以降 12時間 breaker が in-memory で Yahoo を停止し続けた状態で観察。
- **canary log (hourly cron、1req/hour × 単一URL) の推移**:
  - 20:00 200 (正常)
  - **21:00 429** ← Stage 3 発生時のペナルティが hourly canary まで巻き込んだ (初事象)
  - 22:00 200 ← **1時間で復旧**
  - 22:00 〜 翌 11:00 の 13時間: 連続 200、応答時間 0.5-1.5s、サイズ 267KB 正常帯
- **判明した事実**:
  1. Yahoo IP ペナルティは **約1時間程度で解除された** (canary サンプルベース)
  2. 429 のトリガーとなった Stage 3 (20kw + Cold 11kw 集中) は canary まで一時的に巻き込むほど強いペナルティ
  3. ただし解除は速い (6時間半かかった今朝のパターンとは対照的)
- **429 発生時のリカバリ戦略の更新**:
  - **旧設計 (仮定24 由来)**: 「短時間休止は無効、大幅休止 (6時間+) が必要」
  - **新設計 (仮定25 として記録)**: **canary の 200 復帰を確認してから再開する**。固定時間待機ではなく、実測ベースで判断する
- **サンプル数**: 実測 1 回のみ (2026-07-06 21:00 → 22:00)。今朝の 10:50 → 16:50 (6h) は「6時間なら確実に復帰」の観測、今回の 21:00 → 22:00 (1h) は「1時間で復帰することもある」の観測。両方が可能性としてあるため、**canary 確認 = 復帰判定の最良指標** という結論
- **影響範囲**:
  - 実装コード変更なし (canary は既存 cron 稼働、追加ロジックなし)
  - 運用手順の更新: 429 発生時は canary log を確認して復帰判定
