...
- 案E（filter.matchesKeyword緩和）: 先頭Nトークン一致への変更検討中
  （Case D で「Mercariは緩い部分マッチ / filter は AND完全一致」の非対称が判明。
  dry-runシミュレーション実施予定、実装は24時間観察結果を踏まえて判断）
- 案F（tier別layer_a_max_hours調整）: Cold/StarredOosの経過時間制限を
  緩和する案。id=55/65 ナイスリム系の調査で「死んでいるキーワードの一部は
  サンプリング偶然（48h以内出品を逃した）」と判明したため、tier ごとに
  max_hours を延長 (Cold=168h, StarredOos=168h 等) する案を検討
...
- Yahoo!フリマ: 7kw allowlist + 階層化スキャンで安定運用確立済み。
  2026-07-09時点でキーワード短縮（案A、容量保持ルール付き）も完了し、
  新規kw通知シェアがさらに改善見込み（24時間観察中、直後30分で通知+46件を確認）。
