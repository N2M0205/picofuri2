...
## 6. 重要な限界（2026-07-08判明）
canary監視（1req/hour）は、実スキャン（並列複数リクエスト）と
負荷プロファイルが異なるため、canaryが200を返している間でも
実スキャン側でbreakerが発動する可能性がある。実際に2026-07-08
00:02にこの事象が発生し、約9.5時間気づかれずにYahoo停止状態が
続いた。canaryはあくまで参考指標であり、breaker発動の監視は
別途（Telegram通知の確実な到達確認等）で行う必要がある。

### 事象の詳細
- 2026-07-08 00:01/00:02 JST に 429 が 2 連続検出、cascading breaker 発動
- 同時間帯の canary は 200 を返却（毎時 :00 の 1req/hour）
- breaker 発動時に Telegram 通知は正常送信された模様 (エラーログなし) だが、
  深夜のため owner が見落とし
- 翌日 09:40 (owner 依頼の Yahoo allowlist 調整による PM2 restart) まで
  Yahoo 停止状態が継続した

### 監視の運用対策 (2026-07-08 策定)
1. **breaker 状態の複数手段でのクロスチェック**:
   - grep で単一ファイルずつ調べる (複数ファイル指定時の filename prefix に注意)
     または `grep -h` フラグで prefix 抑制
   - 補助スクリプト `scripts/check-yahoo-breaker.js` を使い、
     過去N時間の breaker 状態を安全に確認
2. **Telegram 通知の到達確認**:
   - 現状 breaker 発動時に aicham_dev_bot 経由で通知送信
   - 深夜帯 or 通知埋没への対策として、以下いずれかを検討:
     - 発動 30 分後に自動リマインダー再送
     - 発動を picofuri2_bot (仕入通知用 Bot) にも投げて二重化
     - チャットの ピン留め通知に昇格
3. **セッション開始時の棚卸し**:
   - CLAUDE.md「セッション開始時の棚卸し」項目に「直近 breaker 発動有無の確認」を
     必須項目として追加すること
